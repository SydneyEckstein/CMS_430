# Challenge Project: Conversational AI Philosopher
...
1. **Vector Database** — stores chunked text pieces from the philosopher's works. When it's time to construct a response, the system pulls relevant text chunks and adds them to the response-generation prompt. The technique of prompting an LLM with additional context drawn from a database is called retrieval-augmented generation (RAG) and is a fundamental technique for building AI systems.

2. **State Machine** — tracks the flow of the conversation. It manages the current topic of discussion, the student's level of understanding within that topic, and the philosopher's in-character mood and response style. As the user inputs answers, the state can change, which represents an evolution of the conversation.

3. **LLM-based Text Classifier** — takes a user input and maps it to one of a small number of categories. The text classifications are used to drive transitions in the state machine.

4. **Response Generator** — takes the last user input, the current state, and any RAG context and creates a prompt. An LLM then uses the prompt to create the character's next response.
...
It's important to clarify that you are creating a character, a fictional interpretation of a real person.

We can't really know, after all, what Socrates was like, whether Nietzsche had a sense of humor (probably not), or whether Kant was actually good at explaining his own ideas. Historians do agree, though, that Kierkegaard was a total bishōnen.
...
Start by doing some background reading on RAG. Here's a good starting overview.
...
- Directions in latent space represent conceptual relationships
...
3. Retrieve their associated text chunks from the database
4. Incorporate that text into the LLM prompt

### Tools

I recommend using **ChromaDB** as your database. You can install it locally. Use **sentence-transformers** for embedding. Work with Claude to build a minimal viable example to practice working with these tools before you continue with the rest of the project.
...
> Note that these must be **two separate LLM calls**. You can't use one call for both classification and generation!
...
At minimum, you must implement a different visual representation of the philosopher for each tone state. Discuss your choices in your character document. You can choose any visual style or representation you think is appropriate.
